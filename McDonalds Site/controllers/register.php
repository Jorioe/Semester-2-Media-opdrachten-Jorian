<?php
    ...
    $_first_name = $_last_name = $_email = $_username = $_password = "";
    ...
    if(isset($_POST["submit"])) {
        ...
        if(!empty($firstname) && !empty($lastname) && !empty($email) && !empty($username) && !empty($password)){
            ...
            if($rowCount > 0) {
                ...
                if(!preg_match("/^[a-zA-Z0-9]{6,20}*$/", $_username)) {
                    $u_NameErr = '<div class="alert alert-danger">
                    Usernames should be between 6 to 20 charcters long. Only letters and digits are allowed.
                        </div>';
                }
                ...
                if((preg_match("/^[a-zA-Z ]*$/", $_first_name)) && (preg_match("/^[a-zA-Z ]*$/", $_last_name)) &&
                 (filter_var($_email, FILTER_VALIDATE_EMAIL)) && (preg_match("/^[a-zA-Z0-9]{6,20}*$/", $_username)) && 
